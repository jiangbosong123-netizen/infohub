...
| [美股覆盖与边界](docs/spec/US_MARKET.md) | 当前13家关注公司、消息来源、覆盖缺口与下一步 |
| [时间契约](docs/spec/TIME_CONTRACT.md) | 发布/接收/观察/分析/可见时间、历史知识检查点与市场日历 |
...
**系统由情报数据底座、持续处理流水线、门户网站、对外 API 四部分组成。** 数据库是核心存储；门户长期保留，负责阅读、检索、证据核查和人工复核。内部架构演进不会把网站取消，也不让门户成为第二套事实存储。
...
目前关注13家美国上市公司：NVDA、AAPL、MSFT、GOOGL、META、AMZN、TSLA、AVGO、AMD、TSM、ORCL、PLTR、ARM，已存在SEC文件、公司Google News聚合和财经媒体信息。当前样本偏科技公司，不能称为全美股覆盖；stock频道也不是纯美股。详细现状与证据见[美股覆盖](docs/spec/US_MARKET.md)。

先保存这部分覆盖并补齐原始申报、时间和身份语义，再按来源扩展行业/宏观。门户按实体及上市关系筛选；NLP将公司事实、媒体情绪、推断影响分开。行情和交易职责仍属于外部系统。
...
- **INV-04 时间分开。** 发布、事件发生、系统首次看到、分析完成和消费者可见时间分别记录。`available_at` 表示应用发布事务时间；严格历史使用还须已提交知识检查点与时钟证据，不能用格式精确冒充实际可见时间。
...
本次补查确认美股消息已有覆盖，并新增[美股分册](docs/spec/US_MARKET.md)、[时间分册](docs/spec/TIME_CONTRACT.md)及固定时间验收案例。审计A31–A33纳入路线图；API增加来源时间溯源、知识检查点、窗口与证券身份关系。文档、机器契约、示例与链接由[校验脚本](docs/spec/evidence/validate_docs.py)共同检查。

| 检查项 | 本版结论 | 后续放行条件 |
|---|---|---|
| 产品边界 | 情报底座＋流水线＋门户＋API；美股科技关注范围明确 | 扩大市场范围先登记来源/样本/成本，不按频道名宣称全覆盖 |
| 数据与证据 | 文章/事件/主题/实体分开，版本追加，原文与AI分开 | P06–P10完成原始记录、身份、修正与历史映射 |
| 时间 | 发布/更新/申报/观察/处理/可见分开；严格历史加已提交检查点 | 冻结案例由对应PR实现，Windows时钟与真实来源格式另验 |
| NLP | 对象、方向、方面、期限、证据、模型/prompt版本和评估可执行 | gold与指标实际达标，不能用文档校验替代模型质量 |
| API | 类型契约＋权限＋版本/快照/changes＋历史读取边界 | P17–P19契约与真实消费者演练；未部署接口不宣传可用 |
| 生产数据 | 保留旧ID/字段/缺失状态，先导出备份再副本演练 | 最新Windows库与恢复证据仍为硬门槛；本版不批准直接迁库 |
| 实施粒度 | P01–P23按依赖，小PR可回滚；P06进一步拆a/b/c | 先R1安全底座，再证据/时间，随后模型与门户完善 |

**本SPEC已具备启动R1实施的设计细节；这不等于已批准生产数据迁移或已实现目标功能。** 实施交接以审阅通过的SPEC提交为基线。PR #1继续不原样合并；本次只有文档与离线审查材料，没有运行时功能或生产部署变更。
