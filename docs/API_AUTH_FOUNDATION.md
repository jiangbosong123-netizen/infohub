...
**当前没有给真实消费者发密钥。** `/api/v1` 请求先经过默认拒绝的 Bearer 入口，只有目标
契约列出的只读方法和路径能够进入校验；尚未实现的路由在校验后仍返回 404。P18a 首个
`GET /api/v1/entities` 读取路由也受独立功能开关保护，默认返回 503，完成数据契约与生产
验收后才开启。缺失/无效/撤销密钥返回不含敏感内容的 401，权限不足返回 403，鉴权数据库
不可用返回 503；请求 ID 与错误结构遵循目标契约。未经列明的 v1 路径和方法直接 404。
现有网页和 `/api/health` 等运行探针保持原行为。
