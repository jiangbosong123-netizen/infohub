...
时间前置：NLP输入manifest与输出同时遵守[时间契约](TIME_CONTRACT.md)。analyzed_at、available_at和选材as_of分别记录；运行今天的模型不能使旧新闻在历史时点拥有今天的分析。美股样本必须包含SEC的接收/公开/报告期区分、6-K/20-F/修订表单、ADR/同发行人多市场、盘前盘后、跨时区和迟到报道；US公司现有关联不是gold标签。严格历史指标只使用检查点限制的版本闭包；晚到与人工改正产生新版本。
