...
此步只切换**展示字段**。列表可见性、精选排序、分类过滤、搜索索引、热点计算、
日报以及旧 AI 写入仍读取 `items`，因此在新旧结果不一致时它们可能与卡片文本不同。
在这些查询迁移并通过回归前不要在生产环境开启此开关；尤其不能把本投影视为
完整的新版发布通道。回滚只需关闭开关并重启 web 进程，无需回退数据库迁移。
